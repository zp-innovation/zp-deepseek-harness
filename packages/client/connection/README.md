...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. An unauthenticated index request renders the account login page, and `POST /login` accepts a bounded URL-encoded username and password after the Host/Origin trust check. The SQLite file at `userDatabasePath` stores bcrypt password hashes; an empty database receives `bootstrapUsername` and `bootstrapPassword`, which default to `admin` and `admin123`. A successful login writes an authority-bound signed cookie and redirects to `/`. The process launch-token URL remains an alternate bootstrap path: `dsh-web-app` prints `?token=...`, and `ctx.connection.authorizeIndex` exchanges a valid token only on `GET /`. Missing, expired, malformed, or wrong-authority credentials return 401 before RPC dispatch. Static assets remain public.
...
Before authentication, API and login requests pass `src/api-request-trust.ts`. Their `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. An all-interface bind derives trusted LAN IP literals for account access. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
...
- **The bootstrap password is deployment configuration** — change `bootstrapPassword` before first startup on any network-reachable deployment; changing it after the SQLite database contains an account does not replace that account's password.
