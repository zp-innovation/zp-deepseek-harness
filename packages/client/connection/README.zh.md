...
每个 Host RPC 方法和 WebSocket stream 都要求同一个浏览器会话，不存在按方法区分的 loopback 层。未认证的 index 请求会呈现账号登录页；`POST /login` 在通过 Host/Origin 信任检查后接受有大小上限的 URL 编码用户名和密码。`userDatabasePath` 指向的 SQLite 文件存储 bcrypt 密码哈希；空数据库会创建 `bootstrapUsername` 与 `bootstrapPassword`，其默认值为 `admin` 和 `admin123`。登录成功会写入绑定 authority 的签名 cookie 并重定向到 `/`。进程启动令牌 URL 仍是另一条引导路径：`dsh-web-app` 打印 `?token=...`，`ctx.connection.authorizeIndex` 仅在 `GET /` 交换有效令牌。缺失、过期、畸形或 authority 不匹配的凭据会在 RPC 分发前得到 401。静态资源保持公开。
...
认证之前，API 与登录请求都会经过 `src/api-request-trust.ts`。其 `Host` 必须是 loopback，或与 `trustedHosts` 条目匹配：带端口的 `host:port` 精确匹配，不带端口的条目匹配任意端口，两侧均经 WHATWG 归一化。若附带 `Origin`，它必须等于该 Host；`sec-fetch-site: cross-site` 一律拒绝。畸形配置 authority 会让插件加载失败。这些检查防御 DNS rebinding 与跨站浏览器请求，绝不建立身份。Host/Origin 校验失败返回 403；Host 可信但未认证的请求返回 401。全接口监听会派生受信任的 LAN IP 字面量以供账号访问。决策记录：[浏览器请求信任](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.zh.md)与[浏览器令牌认证](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.zh.md)。
...
- **引导密码属于部署配置**：任何可经网络访问的部署都应在首次启动前修改 `bootstrapPassword`；SQLite 数据库已有账号后再修改该字段，不会替换现有账号的密码。
